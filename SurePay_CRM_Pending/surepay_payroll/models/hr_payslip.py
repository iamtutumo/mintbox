...
class HrPayslip(models.Model):
    ...
    _name = 'surepay_payroll.hr.payslip'
    _inherit = 'mail.thread'
    _description = 'Pay Slip'

    struct_id = fields.Many2one(comodel_name='surepay_payroll.hr.payroll.structure',
                                string='Structure',
    ...
    line_ids = fields.One2many('surepay_payroll.hr.payslip.line',
                               'slip_id',
    ...
    worked_days_line_ids = fields.One2many('surepay_payroll.hr.payslip.worked.days',
                                            'payslip_id',
                                            string='Payslip Worked Days',
                                            copy=True,
                                            help="Payslip worked days for line")
    input_line_ids = fields.One2many('surepay_payroll.hr.payslip.input',
                                      'payslip_id',
    ...
    details_by_salary_rule_category_ids = fields.One2many(
        comodel_name='surepay_payroll.hr.payslip.line',
    ...
    credit_note = fields.Boolean(string='Credit Note',
                                 help="Indicates this payslip has "
                                      "a refund of another")
    payslip_run_id = fields.Many2one('surepay_payroll.hr.payslip.run',
    ...
    is_send_mail = fields.Boolean(string='Send Mail', default=False,
                                 help="Check to send payslip by email")
    gross_wage = fields.Float(compute='_compute_gross_wage', string='Gross Wage',
                              help="Total gross wage amount", store=True)
    net_wage = fields.Float(compute='_compute_net_wage', string='Net Wage',
                            help="Total net wage amount", store=True)
    total_ded = fields.Float(compute='_compute_total_ded', string='Total Deductions',
                             help="Total deductions amount", store=True)
    ...
    def _compute_gross_wage(self):
        """Compute function for getting total gross wage amount"""
        for payslip in self:
            gross_lines = payslip.line_ids.filtered(lambda line: line.category_id.code == 'GROSS')
            payslip.gross_wage = sum(gross_lines.mapped('total')) if gross_lines else 0.0

    @api.depends('line_ids.total', 'line_ids.category_id')
    def _compute_net_wage(self):
        """Compute function for getting total net wage amount"""
        for payslip in self:
            net_lines = payslip.line_ids.filtered(lambda line: line.category_id.code == 'NET')
            payslip.net_wage = sum(net_lines.mapped('total')) if net_lines else 0.0

    @api.depends('line_ids.total', 'line_ids.category_id')
    def _compute_total_ded(self):
        """Compute function for getting total deductions amount"""
        for payslip in self:
            ded_lines = payslip.line_ids.filtered(lambda line: line.category_id.code == 'DED')
            payslip.total_ded = sum(abs(line.total) for line in ded_lines if line.total < 0) if ded_lines else 0.0
    ...
    def action_refund_sheet(self):
        ...
        formview_ref = self.env.ref('surepay_payroll.hr_payslip_view_form',
                                    False)
        treeview_ref = self.env.ref('surepay_payroll.hr_payslip_view_tree',
                                    False)
        return {
    ...
    def get_inputs(self, contracts, date_from, date_to):
        ...
        rule_ids = self.env['surepay_payroll.hr.payroll.structure'].browse(
            structure_ids).get_all_rules()
        sorted_rule_ids = [id for id, sequence in
                           sorted(rule_ids, key=lambda x: x[1])]
        inputs = self.env['surepay_payroll.hr.salary.rule'].browse(sorted_rule_ids).mapped(
            'input_ids')
    ...
    @api.model
    def _process_advance_deductions(self, payslip):
        """Process advance deductions for the payslip"""
        advance_deductions = []
        
        # Get active advances for the employee
        advances = self.env['surepay_payroll.hr.salary.advance'].search([
        ...
        for advance in advances:
            # Calculate deduction amount for this payslip period
            deduction_amount = advance._calculate_deduction_amount(payslip.date_from, payslip.date_to)
            
            if deduction_amount > 0:
                # Create deduction record
                deduction = {
                    'name': f"Advance Deduction - {advance.name}",
                    'code': f'ADV_{advance.id}',
                    'amount': deduction_amount,
                    'salary_advance_id': advance.id,
                }
                advance_deductions.append(deduction)
                
                # Update advance outstanding balance
                advance.write({
                    'outstanding_balance': advance.outstanding_balance - deduction_amount,
                    'total_repaid': advance.total_repaid + deduction_amount,
                })
                
                # Link payslip line to advance
                payslip_line = self.env['surepay_payroll.hr.payslip.line'].search([
                    ('slip_id', '=', payslip.id),
                    ('code', '=', deduction['code'])
                ])
                if payslip_line:
                    advance.write({
                        'payslip_line_ids': [(4, payslip_line.id)]
                    })
                
                # Check if advance is fully repaid
                if advance.outstanding_balance <= deduction_amount:
                    advance.action_complete()
        
        return advance_deductions

    @api.model
    def _process_loan_deductions(self, payslip):
        """Process loan deductions for the payslip"""
        loan_deductions = []
        
        # Get active loans for the employee
        loans = self.env['surepay_payroll.hr.loan'].search([
            ('employee_id', '=', payslip.employee_id.id),
        ...
        for loan in loans:
            ...
            if deduction_amount > 0:
                ...
                deduction = {
                    ...
                    'code': f'LOAN_{loan.id}',
                    'amount': deduction_amount,
                    'loan_id': loan.id,
                }
                loan_deductions.append(deduction)
                ...
                # Link payslip line to loan (will be created later)
                # This will be handled when the payslip line is created
                
                # Check if loan is fully repaid
                if loan.outstanding_balance <= deduction_amount:
                    loan.action_complete()
    ...
    def _get_payslip_lines(self, contract_ids, payslip_id):
        ...
        payslip = self.env['surepay_payroll.hr.payslip'].browse(payslip_id)
        
        # Process advance deductions
        advance_deductions = self._process_advance_deductions(payslip)
        
        # Process loan deductions
        loan_deductions = self._process_loan_deductions(payslip)
        
        # Add advance deductions to inputs
        for deduction in advance_deductions:
            self.env['surepay_payroll.hr.payslip.input'].create({
        ...
        class InputLine(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date=None):
                ...
                self.env.cr.execute("""
                    ...
                    FROM surepay_payroll_hr_payslip as hp, surepay_payroll_hr_payslip_input as pi
        ...
        class WorkedDays(BrowsableObject):
            ...
            def _sum(self, code, from_date, to_date=None):
                ...
                self.env.cr.execute("""
                    ...
                    FROM surepay_payroll_hr_payslip as hp, surepay_payroll_hr_payslip_worked_days as pi
                    WHERE hp.employee_id = %s AND hp.state = 'done'
                    AND hp.date_from >= %s AND hp.date_to <= %s AND hp.id = 
                    pi.payslip_id AND pi.code = %s""",
                                    (
        ...
        class Payslips(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date=None):
                ...
                FROM surepay_payroll_hr_payslip as hp, surepay_payroll_hr_payslip_line as pl
                WHERE hp.employee_id = %s AND hp.state = 'done'
                AND hp.date_from >= %s AND hp.date_to <= %s AND hp.id 
        ...
        sorted_rule_ids = [id for id, sequence in
                           sorted(rule_ids, key=lambda x: x[1])]
        sorted_rules = self.env['surepay_payroll.hr.salary.rule'].browse(sorted_rule_ids)
        for contract in contracts:
    ...
    def _check_registry(self):
        _logger.info("surepay_payroll hr_payslip loaded, registry has surepay_payroll.hr.payslip: %s", 'surepay_payroll.hr.payslip' in self.env.registry)
        _logger.info("Registry models with payslip: %s", [k for k in self.env.registry.keys() if 'payslip' in k])
        return
