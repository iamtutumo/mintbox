...
class ReportHrPayrollCommunityReportContributionRegister(models.AbstractModel):
    ...
    _name = 'report.surepay_payroll.report_contributionregister'
    ...
    def _get_payslip_lines(self, register_ids, date_from, date_to):
        ...
        result = {}
        self.env.cr.execute("""
            SELECT pl.id from surepay_payroll_hr_payslip_line as pl
            LEFT JOIN surepay_payroll_hr_payslip AS hp on (pl.slip_id = hp.id)
        ...
        for line in self.env['surepay_payroll.hr.payslip.line'].browse(line_ids):
            result.setdefault(line.register_id.id, self.env['surepay_payroll.hr.payslip.line'])
    ...
    def _get_report_values(self, docids, data=None):
        ...
        contrib_registers = self.env['surepay_payroll.hr.contribution.register'].browse(
            register_ids)
        ...
        return {
            'doc_ids': docids,
            'doc_model': 'surepay_payroll.hr.contribution.register',
