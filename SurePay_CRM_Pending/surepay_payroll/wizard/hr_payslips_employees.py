...
class HrPayslipEmployees(models.TransientModel):
    ...
    _name = 'surepay_payroll.hr.payslips.employees'
    _description = 'Generate payslips for all selected employees'

    employee_ids = fields.Many2many('hr.employee',
                                    'surepay_payslip_employee_rel',
                                    'wizard_id',
    ...
    def action_compute_sheet(self):
        ...
        payslips = self.env['surepay_payroll.hr.payslip']
        [data] = self.read()
        active_id = self.env.context.get('active_id')
        if active_id:
            [run_data] = self.env['surepay_payroll.hr.payslip.run'].browse(active_id).read(
        ...
        for employee in self.env['hr.employee'].browse(data['employee_ids']):
            slip_data = (
                self.env['surepay_payroll.hr.payslip'].onchange_employee_id(
            ...
            payslips += self.env['surepay_payroll.hr.payslip'].create(res)
