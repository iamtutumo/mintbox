...
class HrPayrollReportView(models.Model):
    ...
    rule_name = fields.Many2one('surepay_payroll.hr.salary.rule.category',
                                string="Rule Category",
                                help="Choose Salary Rule Category")
    rule_amount = fields.Float(string="Amount", help="Set Amount")
    struct_id = fields.Many2one('surepay_payroll.hr.payroll.structure',
                                string="Salary Structure",
                                help="Choose Hr Payroll Structure")
    rule_id = fields.Many2one('surepay_payroll.hr.salary.rule',
    ...
    def _from(self):
        """
            ...
            """
        from_str = """
                surepay_payroll_hr_payslip_line psl   
                join surepay_payroll_hr_payslip ps on ps.id=psl.slip_id
                join surepay_payroll_hr_salary_rule rlu on rlu.id = psl.salary_rule_id
                join hr_employee emp on ps.employee_id=emp.id
                join surepay_payroll_hr_salary_rule_category rl on rl.id = psl.category_id
