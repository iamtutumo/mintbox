...
_logger.info("About to define HrPayslip class in surepay_payslip_monthly_report")
            ...
            for payslip in self:
                if payslip.employee_id.private_email:
                    template = self.env.ref(
                        'surepay_payslip_monthly_report.email_template_payslip')
    ...
    def action_payslip_send(self):
        ...
        try:
            template_id = ir_model_data._xmlid_lookup(
                'surepay_payslip_monthly_report.email_template_payslip')[1]
        except ValueError:
            template_id = False
        try:
        ...
        ctx = {
            'default_model': 'surepay_payroll.hr.payslip',
